	...
	factory: (require) => {
		...
		function createPanelStore() {
			...
			const empty = () => ({
				...
				/**
				 * 多轮追问：**轮次数组**。
				 * 每轮 = `{ id, question, answer, reasoning, tool, phase, error, newSegment }`。
			...
			const normalizeRound = (raw) => {
				...
				return {
					...
					answer: clampString(round.answer, 200000),
					reasoning: clampString(round.reasoning, 200000),
		...
		const zh = {
			...
			// 引用块左侧那颗「引用」chip 的文字
			'panel.quoteChip': '引用',
			// 工具调用提示行（白名单放行后模型会真的调 read/grep 等，只显示工具名）
			'panel.toolPrefix': '运行：',
		...
		function SideBranchBody(props) {
			...
			/** 答案区元素；流式时必须自动滚到底，否则新内容不可见 */
			const answerRef = react.useRef(null)
			react.useLayoutEffect(() => {
				const node = answerRef.current
				if (node === null || node === undefined) return
				node.scrollTop = node.scrollHeight
				// 依赖：最后一轮的正文 + 推理（多轮下内容都长在最后一轮里）
			}, [rounds.length, rounds[rounds.length - 1]?.answer, rounds[rounds.length - 1]?.reasoning])
			...
			const openStream = (mine) => {
				...
				on('delta', (event) => {
					...
					// ★ 逐词追加：读当前值再拼（不依赖渲染态，避免丢字）
					const current = currentRound()
					patchRound({ answer: (current?.answer ?? '') + payload.text })
				})
				...
				on('reasoning', (event) => {
					...
					if (payload === undefined || typeof payload.text !== 'string' || payload.text === '') return
					const current = currentRound()
					patchRound({ reasoning: (current?.reasoning ?? '') + payload.text })
				})
				on('reset', () => {
					if (jobRef.current !== mine) return
					// ⚠️ 重试（新的 attempt）也要把工具提示清掉，否则上一轮的工具名会挂着
					patchRound({ answer: '', reasoning: '', tool: '' })
				})
				// ★ 工具调用提示：宿主只推工具名
				on('tool', (event) => {
					if (jobRef.current !== mine) return
					...
					patchRound({ tool: payload.name })
				})
				on('replace', (event) => {
					if (jobRef.current !== mine) return
					const payload = parseEvent(event)
					if (payload !== undefined && typeof payload.text === 'string') patchRound({ answer: payload.text })
				})
			...
			const ask = async () => {
				...
				const composed = buildReferenceEnvelope(text, quoted, t)
				closeStream()
				jobRef.current = null
				const round = {
				...
				try {
					...
					write({
						...
						// ⚠️ 换模型时宿主会**带历史**换段并回一个新的 id（`handoff: true`）——
						//    那一段里含上面所有轮次，所以**不画分隔线**、历史在视觉上连续。
			...
			/**
			 * 一轮的正文渲染。
			 * @param {object} round - 一轮
			 * @param {boolean} streaming - 这一轮是否仍在流式（只有最后一轮可能为真）
			 */
			const renderAnswer = (round, streaming) => {
				...
				if (MarkdownText != null) {
					return react.createElement(
						...
						MarkdownBoundary,
						{ key: 'md', text: round.answer },
						react.createElement(MarkdownText, { text: round.answer, streaming, labels }),
					)
				}
				return react.createElement(
					'div',
			...
			/** 一轮的完整渲染：问题气泡 →（可选）可折叠「思考」行 →（可选）工具行 → 答案 → 轮次小标签。 */
			const renderRound = (round, index) => {
				...
				//   换模型 = `handoff` 带历史换段、**不画**；「清空」把面板清空（`rounds` 为 0）、
				//   闲置唤醒是同一个 id ⇒ 这两种也不会画。
				...
				const hasReasoning = typeof round.reasoning === 'string' && round.reasoning !== ''
				if (hasReasoning) {
					children.push(react.createElement(ReasoningRow, { key: 'think', text: round.reasoning, running: streaming, t }))
				}
				// ②b 工具调用提示行（只在运行中显示；终态后自动消失）
				if (streaming === true && typeof round.tool === 'string' && round.tool !== '') {
					children.push(
						react.createElement('div', { key: 'tool', className: 'dsh-side-branch-toolline' }, t('panel.toolPrefix') + round.tool),
					)
				}
				// ③ 正文（还没出字就什么都不画：运行状态由工具行的**停止键**表达）
				children.push(
					react.createElement(
						'div',
						{ key: 'a' },
						round.answer === '' ? '' : renderAnswer(round, streaming),
					),
				)
			...
			return react.createElement(
				...
				react.createElement('div', { className: 'dsh-side-branch-scroll' }, errorBlock, answerBlock),
				composer,
				settingsPanel,
				usageLine,
			)
		...
		function selectionText(selection) {
			...
			const refused = 'input, textarea, button, [contenteditable]:not([contenteditable="false"]), [hidden]'
						...
						ctx.slots.register(
							{
								...
								inject: () => ({ startSideBranch, streamUrl, stopSideBranch, closeSideBranch, listModels, fetchSettings, saveSettings }),
							},
							SideBranchBody,
						),
					),
